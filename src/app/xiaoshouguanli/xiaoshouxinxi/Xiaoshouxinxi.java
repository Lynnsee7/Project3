...
public class Xiaoshouxinxi extends BaseFormController {
			...
			if (jsonAl != null && jsonAl.size() > 0) {
				for (int i = 0; i < jsonAl.size(); i++) {
					...
					if (kucun - shuliang_xiugaihou < 0) {
						throw new Exception("货号为：" + huohao + "的库存量不足！");
					} else {
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
		...
		// 表头(列号,行号 ,内容 )
		Label label_0 = new Label(1, 3, "销售单号", wcfFC5);
		ws.addCell(label_0);
		Label label_1 = new Label(2, 3, "经手人", wcfFC5);
		ws.addCell(label_1);
		Label label_2 = new Label(3, 3, "消费金额", wcfFC5);
		ws.addCell(label_2);
		Label label_3 = new Label(4, 3, "会员编号", wcfFC5);
		ws.addCell(label_3);
