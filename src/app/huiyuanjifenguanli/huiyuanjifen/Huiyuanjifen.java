package app.huiyuanjifenguanli.huiyuanjifen;
...
public class Huiyuanjifen extends BaseFormController {
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (flag != null && flag.equals("getJsonStore")) {
				...
				return toFinish("/WEB-ROOT/app/huiyuanjifenguanli/huiyuanjifen/huiyuanjifen.jsp", resultMap, request, response);
			}
	...
	public ModelAndView doGetJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String huiyuanmingchengSearch = request.getParameter("huiyuanmingchengSearch");
			if (huiyuanmingchengSearch != null && !huiyuanmingchengSearch.equals("")) {
				where.put("huiyuanmingchengSearch", huiyuanmingchengSearch);
			}
			String huiyuanjifenSearch = request.getParameter("huiyuanjifenSearch");
			if (huiyuanjifenSearch != null && !huiyuanjifenSearch.equals("")) {
				where.put("huiyuanjifenSearch", huiyuanjifenSearch);
			}
			...
			}else if (r != null && r.equals("y")) {

				if (SysInfo.getLoginUserType(request, response).equals("2")) {
					where.put("dataRight", "  operatorId='" + SysInfo.getLoginUserId(request, response) + "' or huiyuanmingcheng='"+SysInfo.getLoginUserAcct(request, response)+"' ");
				}

			}

			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Huiyuanjifen.selecteList");
	...
	public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String huiyuanmingcheng = request.getParameter("huiyuanmingcheng");
			where.put("huiyuanmingcheng", huiyuanmingcheng);
			String huiyuanjifen = request.getParameter("huiyuanjifen");
			where.put("huiyuanjifen", huiyuanjifen);
			String detail = request.getParameter("detail");
			where.put("detail", detail);

			sqlMap.insert("Huiyuanjifen.insertObj", where);
	...
	public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			sqlMap.insert("Huiyuanjifen.insertObj", whereMap);
	...
	public ModelAndView doUpdate(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			sqlMap.update("Huiyuanjifen.updateObj", where);
	...
	public ModelAndView doUpdateWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			whereMap.put("id", id);
			sqlMap.update("Huiyuanjifen.updateObj", whereMap);
	...
	public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			where.put("id", id);
			sqlMap.delete("Huiyuanjifen.doDeleOrUnDele", where);
	...
	public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			where.put("id", id);
			sqlMap.delete("Huiyuanjifen.doShenPi", where);
	...
	public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Huiyuanjifen.selecteList", where);
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
		...
		// 表头(列号,行号 ,内容 )
		Label label_0 = new Label(1, 3, "会员名称", wcfFC5);
		ws.addCell(label_0);
		Label label_1 = new Label(2, 3, "会员积分", wcfFC5);
		ws.addCell(label_1);
		Label label_2 = new Label(3, 3, "Id", wcfFC5);
		ws.addCell(label_2);
		Label label_3 = new Label(4, 3, "创建时间", wcfFC5);
		ws.addCell(label_3);
		Label label_4 = new Label(5, 3, "备注", wcfFC5);
		ws.addCell(label_4);

		// 表体
		for (int i = 0; i < al.size(); i++) {
			String huiyuanmingcheng = ((HashMap) (al.get(i))).get("huiyuanmingcheng").toString();
			label_0 = new Label(1, i + 4, huiyuanmingcheng, wcfF4);
			ws.addCell(label_0);
			String huiyuanjifen = ((HashMap) (al.get(i))).get("huiyuanjifen").toString();
			label_1 = new Label(2, i + 4, huiyuanjifen, wcfF4);
			ws.addCell(label_1);
			String id = ((HashMap) (al.get(i))).get("id").toString();
			label_2 = new Label(3, i + 4, id, wcfF4);
			ws.addCell(label_2);
			String itime = ((HashMap) (al.get(i))).get("itime").toString();
			label_3 = new Label(4, i + 4, itime, wcfF4);
			ws.addCell(label_3);
			String detail = ((HashMap) (al.get(i))).get("detail").toString();
			label_4 = new Label(5, i + 4, detail, wcfF4);
			ws.addCell(label_4);
		}
