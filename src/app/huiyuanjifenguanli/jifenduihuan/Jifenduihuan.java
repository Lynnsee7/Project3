package app.huiyuanjifenguanli.jifenduihuan;
...
public class Jifenduihuan extends BaseFormController {
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (flag != null && flag.equals("getJsonStore")) {
				...
				return toFinish("/WEB-ROOT/app/huiyuanjifenguanli/jifenduihuan/jifenduihuan.jsp", resultMap, request, response);
			}
	...
	public ModelAndView doGetJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String huiyuanhaoSearch = request.getParameter("huiyuanhaoSearch");
			if (huiyuanhaoSearch != null && !huiyuanhaoSearch.equals("")) {
				where.put("huiyuanhaoSearch", huiyuanhaoSearch);
			}
			String duihuanwupinSearch = request.getParameter("duihuanwupinSearch");
			if (duihuanwupinSearch != null && !duihuanwupinSearch.equals("")) {
				where.put("duihuanwupinSearch", duihuanwupinSearch);
			}
			String duihuanriqiSearch = request.getParameter("duihuanriqiSearch");
			if (duihuanriqiSearch != null && !duihuanriqiSearch.equals("")) {
				where.put("duihuanriqiSearch", duihuanriqiSearch);
			}
			String shiyongjifenSearch = request.getParameter("shiyongjifenSearch");
			if (shiyongjifenSearch != null && !shiyongjifenSearch.equals("")) {
				where.put("shiyongjifenSearch", shiyongjifenSearch);
			}
			...
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Jifenduihuan.selecteList");
	...
	public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String huiyuanhao = request.getParameter("huiyuanhao");
			where.put("huiyuanhao", huiyuanhao);
			String duihuanwupin = request.getParameter("duihuanwupin");
			where.put("duihuanwupin", duihuanwupin);
			String duihuanriqi = request.getParameter("duihuanriqi");
			where.put("duihuanriqi", duihuanriqi);
			String shiyongjifen = request.getParameter("shiyongjifen");
			where.put("shiyongjifen", shiyongjifen);
			String detail = request.getParameter("detail");
			where.put("detail", detail);
			
			
			
			where.put("jifen", Integer.parseInt(shiyongjifen));
			where.put("huiyuanbianhao", huiyuanhao);
			int num = sqlMap.update("Huiyuanjifen.updateObj3", where);
			if( num == 0 ) {
				throw new Exception("该会员编号不存或积分不够，请核实！");
			}else {
				sqlMap.insert("Jifenduihuan.insertObj", where);
	...
	public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			sqlMap.insert("Jifenduihuan.insertObj", whereMap);
	...
	public ModelAndView doUpdate(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			sqlMap.update("Jifenduihuan.updateObj", where);
	...
	public ModelAndView doUpdateWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			whereMap.put("id", id);
			sqlMap.update("Jifenduihuan.updateObj", whereMap);
	...
	public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			where.put("id", id);
			sqlMap.delete("Jifenduihuan.doDeleOrUnDele", where);
	...
	public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			where.put("id", id);
			sqlMap.delete("Jifenduihuan.doShenPi", where);
	...
	public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Jifenduihuan.selecteList", where);
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
		...
		// 表头(列号,行号 ,内容 )
		Label label_0 = new Label(1, 3, "会员号", wcfFC5);
		ws.addCell(label_0);
		Label label_1 = new Label(2, 3, "兑换物品", wcfFC5);
		ws.addCell(label_1);
		Label label_2 = new Label(3, 3, "兑换日期", wcfFC5);
		ws.addCell(label_2);
		Label label_3 = new Label(4, 3, "使用积分", wcfFC5);
		ws.addCell(label_3);
		...
		for (int i = 0; i < al.size(); i++) {
			String huiyuanhao = ((HashMap) (al.get(i))).get("huiyuanhao").toString();
			label_0 = new Label(1, i + 4, huiyuanhao, wcfF4);
			ws.addCell(label_0);
			String duihuanwupin = ((HashMap) (al.get(i))).get("duihuanwupin").toString();
			label_1 = new Label(2, i + 4, duihuanwupin, wcfF4);
			ws.addCell(label_1);
			String duihuanriqi = ((HashMap) (al.get(i))).get("duihuanriqi").toString();
			label_2 = new Label(3, i + 4, duihuanriqi, wcfF4);
			ws.addCell(label_2);
			String shiyongjifen = ((HashMap) (al.get(i))).get("shiyongjifen").toString();
			label_3 = new Label(4, i + 4, shiyongjifen, wcfF4);
			ws.addCell(label_3);
