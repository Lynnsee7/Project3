package app.huiyuanguanli.huiyuanxinxi;
...
public class Huiyuanxinxi extends BaseFormController {
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (flag != null && flag.equals("getJsonStore")) {
				...
				return toFinish("/WEB-ROOT/app/huiyuanguanli/huiyuanxinxi/huiyuanxinxi.jsp", resultMap, request, response);
			}
	...
	public ModelAndView doGetJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Huiyuanxinxi.selecteList");
	...
	public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			sqlMap.insert("Huiyuanxinxi.insertObj", where);
	...
	public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (item.isFormField()) {
					if (i == 0) {
						String huiyuanmingcheng = item.getString("gb2312");
						whereMap.put("huiyuanmingcheng", huiyuanmingcheng);
						
						HashMap<String, Object> where = new HashMap<String, Object>();
						where.put("operatorId", SysInfo.getLoginUserId(request, response));
						where.put("huiyuanmingcheng", huiyuanmingcheng);
						where.put("huiyuanjifen", "0");
						where.put("detail", "");
						sqlMap.insert("Huiyuanjifen.insertObj", where);
						
					} else if (i == 1) {
						whereMap.put("huiyuanbianhao", item.getString("gb2312"));
					} else if (i == 2) {
			...
			sqlMap.insert("Huiyuanxinxi.insertObj", whereMap);
	...
	public ModelAndView doUpdate(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			sqlMap.update("Huiyuanxinxi.updateObj", where);
	...
	public ModelAndView doUpdateWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (item.isFormField()) {
					if (i == 0) {
						whereMap.put("huiyuanmingcheng", item.getString("gb2312"));
					} else if (i == 1) {
						whereMap.put("huiyuanbianhao", item.getString("gb2312"));
					} else if (i == 2) {
						whereMap.put("lianxidianhua", item.getString("gb2312"));
			...
			whereMap.put("id", id);
			sqlMap.update("Huiyuanxinxi.updateObj", whereMap);
	...
	public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			where.put("id", id);
			sqlMap.delete("Huiyuanxinxi.doDeleOrUnDele", where);
	...
	public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			where.put("id", id);
			sqlMap.delete("Huiyuanxinxi.doShenPi", where);
	...
	public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Huiyuanxinxi.selecteList", where);
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
		...
		ws.addCell(label_0);
		Label label_1 = new Label(2, 3, "会员编号", wcfFC5);
		ws.addCell(label_1);
		Label label_2 = new Label(3, 3, "联系电话", wcfFC5);
		ws.addCell(label_2);
		...
		for (int i = 0; i < al.size(); i++) {
			...
			label_1 = new Label(2, i + 4, huiyuanbianhao, wcfF4);
			ws.addCell(label_1);
			String lianxidianhua = ((HashMap) (al.get(i))).get("lianxidianhua").toString();
			label_2 = new Label(3, i + 4, lianxidianhua, wcfF4);
			ws.addCell(label_2);
