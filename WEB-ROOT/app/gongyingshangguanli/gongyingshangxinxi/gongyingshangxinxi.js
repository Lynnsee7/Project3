				...
				items : [
					{
						...
						xtype : 'textfield',
						fieldLabel : '供应商名称',
						name : 'gongyingshangmingchengSearch',
						id : 'gongyingshangmingchengSearch',
						anchor : '100%'
					} ]
			}
				,
					...
					items : [
						{
							xtype : 'textfield',
							fieldLabel : '供货商品',
							name : 'gonghuozhouqiSearch',
							id : 'gonghuozhouqiSearch',
							anchor : '100%'
						} ]
				}
				,
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		remoteSort : true,
		fields : [ 'gongyingshangmingcheng', 'dizhi', 'lianxiren', 'gonghuozhouqi', 'lianxidianhua', 'id', 'itime', 'detail', 'deleteFlag' ],
		proxy : new Ext.data.HttpProxy({
			url : '/WEB-ROOT/app/gongyingshangguanli/gongyingshangxinxi.do',
			method : 'POST'
		}),
		baseParams : {
			flag : 'getJsonStore',
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		...
		bbar : pagingBar,
		columns : [ {
			header : '<font color="blue">供应商名称</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'gongyingshangmingcheng'
		}
			, {
			...
			, {
				header : '<font color="blue">供货商品</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'gonghuozhouqi'
			}
			, {
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.gongyingshangmingchengSearch = document.all.gongyingshangmingchengSearch.value;
	...
	ManageObj.dataStore.baseParams.gonghuozhouqiSearch = document.all.gonghuozhouqiSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					...
					name : 'gongyingshangmingcheng',
					id : 'gongyingshangmingcheng',
					anchor : '95%',
					allowBlank : false
				} ]
			}
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('gongyingshangmingcheng').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('gongyingshangmingcheng');
		...
		ManageObj.operationFormPan.findById('gonghuozhouqi').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('gonghuozhouqi');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/gongyingshangguanli/gongyingshangxinxi.do?flag=doExportExcel";
	window.open(url);
};
