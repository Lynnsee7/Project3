				...
				items: [
					{
						xtype: 'textfield',
						fieldLabel: '统计时间',
						name: 'tongjishijianSearch',
						id: 'tongjishijianSearch',
						anchor: '100%'
					}]
			}
				,
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		remoteSort: true,
		fields: ['tongjishijian', 'id', 'itime', 'detail', 'deleteFlag'],
		proxy: new Ext.data.HttpProxy({
			url: '/WEB-ROOT/app/caiwuguanli/xiaoshoutubiaotongji.do',
			method: 'POST'
		}),
		baseParams: {
			flag: 'getJsonStore',
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		...
		bbar: pagingBar,
		columns: [ {
				header: '<font color="blue">统计时间</font>',
				width: 60,
				align: 'center',
				sortable: true,
				dataIndex: 'tongjishijian'
			}
			, {
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.tongjishijianSearch = document.all.tongjishijianSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
	ManageObj.operationFormPan = new Ext.FormPanel({
		...
		items: [{
			...
			items: [{
				...
				items: [{
					xtype: 'textfield',
					fieldLabel: '<font color="red">*</font>&nbsp;&nbsp;统计时间',
					name: 'tongjishijian',
					id: 'tongjishijian',
					anchor: '95%',
					allowBlank: false
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('tongjishijian').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('tongjishijian');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/caiwuguanli/xiaoshoutubiaotongji.do?flag=doExportExcel";
	window.open(url);
};
