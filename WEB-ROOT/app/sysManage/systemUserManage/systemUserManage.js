var ManageObj = function() {
	...
	/* 新增编辑--用户类型store */
	ManageObj.userTypeStore = new Ext.data.SimpleStore({
		fields : [ 'userTypeId', 'userTypeText' ],
		data : [ [ '1', '系统管理员' ], [ '2', '普通用户' ], [ '3', '超市员工' ] ]
	});
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		remoteSort : true,
		fields : [ 'id', 'userAcct', 'userName', 'userSex', 'userType', 'userAge', 'userPass', 'userDoc', 'userDetail', 'userPhone', 'registerTime', 'deleteFlag', 'danwei', 'bumen', 'zhiwei', 'idcard','status' ],
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		...
		columns : [ {
			...
			header : "<font color='blue'>部门</font>",
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'bumen'
		}, {
			header : "<font color='blue'>职位</font>",
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'zhiwei',
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.bumenSearch = document.all.bumenSearch.value;
	ManageObj.dataStore.baseParams.zhiweiSearch = document.all.zhiweiSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
	...
	if (rowIndex != null) {
		...
		ManageObj.operationFormPan.findById('bumen').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('bumen');
		ManageObj.operationFormPan.findById('zhiwei').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('zhiwei');
		ManageObj.operationFormPan.findById('idcard').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('idcard');

		ManageObj.operationFormPan.findById('userType').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('userType');
