				...
				items: [
					{
						xtype: 'textfield',
						fieldLabel: '销售价格',
						name: 'xiaoshoujiageSearch',
						id: 'xiaoshoujiageSearch',
						anchor: '100%'
					}]
			}
				,
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		totalProperty: 'totalProperty',
		remoteSort: true,
		fields: ['huohao', 'xiaoshoujiage', 'shezhiriqi', 'id', 'itime', 'detail', 'deleteFlag'],
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		...
		columns: [{
			...
			, {
				header: '<font color="blue">设置日期</font>',
				width: 60,
				align: 'center',
				sortable: true,
				dataIndex: 'shezhiriqi'
			}
			, {
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.xiaoshoujiageSearch = document.all.xiaoshoujiageSearch.value;
	ManageObj.dataStore.baseParams.shezhiriqiSearch = document.all.shezhiriqiSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
	ManageObj.operationFormPan = new Ext.FormPanel({
		...
		items: [{
			...
			items: [{
				...
				items: [{
					xtype: 'textfield',
					fieldLabel: '<font color="red">*</font>&nbsp;&nbsp;销售价格',
					name: 'xiaoshoujiage',
					id: 'xiaoshoujiage',
					anchor: '95%',
					allowBlank: false
				...
				items: [{
					xtype: 'datefield',
					format: 'Y-m-d',
					fieldLabel: '<font color="red">*</font>&nbsp;&nbsp;设置日期',
					name: 'shezhiriqi',
					id: 'shezhiriqi',
					anchor: '95%',
					allowBlank: false
	...
	if (rowIndex != null) {
		...
		ManageObj.operationFormPan.findById('xiaoshoujiage').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('xiaoshoujiage');
		ManageObj.operationFormPan.findById('shezhiriqi').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('shezhiriqi');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/xiaoshouguanli/jiageshezhi.do?flag=doExportExcel";
	window.open(url);
};
