		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					...
					xtype : 'textfield',
					fieldLabel : '员工姓名',
					name : 'yuangongxingmingSearch',
					id : 'yuangongxingmingSearch',
					anchor : '100%'
				} ]
			}, {
				columnWidth : .2,
				...
				items : [ {
					...
					xtype : 'textfield',
					fieldLabel : '员工工号',
					name : 'yuangonggonghaoSearch',
					id : 'yuangonggonghaoSearch',
					anchor : '100%'
				} ]
			}, {
				columnWidth : .2,
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '入职时间',
					name : 'ruzhishijianSearch',
					id : 'ruzhishijianSearch',
					anchor : '100%'
				} ]
			}, {
				columnWidth : .2,
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '家庭住址',
					name : 'jiatingzhuzhiSearch',
					id : 'jiatingzhuzhiSearch',
					anchor : '100%'
				} ]
			}, {
				columnWidth : .2,
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		remoteSort : true,
		fields : [ 'yuangongxingming', 'yuangonggonghao', 'ruzhishijian',
				'jiatingzhuzhi', 'lianxidianhua', 'id', 'itime', 'detail',
				'deleteFlag' ],
		proxy : new Ext.data.HttpProxy({
			url : '/WEB-ROOT/app/renliziyuanguanli/neibuyuangongguanli.do',
			method : 'POST'
		}),
		baseParams : {
			flag : 'getJsonStore',
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		...
		bbar : pagingBar,
		columns : [ {
			header : '<font color="blue">员工姓名</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'yuangongxingming'
		}, {
			header : '<font color="blue">员工工号</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'yuangonggonghao'
		}, {
			header : '<font color="blue">入职时间</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'ruzhishijian'
		}, {
			header : '<font color="blue">家庭住址</font>',
			width : 60,
			align : 'center',
			sortable : true,
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.yuangongxingmingSearch = document.all.yuangongxingmingSearch.value;
	ManageObj.dataStore.baseParams.yuangonggonghaoSearch = document.all.yuangonggonghaoSearch.value;
	ManageObj.dataStore.baseParams.ruzhishijianSearch = document.all.ruzhishijianSearch.value;
	ManageObj.dataStore.baseParams.jiatingzhuzhiSearch = document.all.jiatingzhuzhiSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
	ManageObj.operationFormPan = new Ext.FormPanel({
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;员工姓名',
					name : 'yuangongxingming',
					id : 'yuangongxingming',
					anchor : '95%',
					allowBlank : false
				} ]
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;员工工号',
					name : 'yuangonggonghao',
					id : 'yuangonggonghao',
					anchor : '95%',
					allowBlank : false
				} ]
				...
				items : [ {
					...
					format : 'Y-m-d',
					invalidText : '日期格式不正确，应为yyyy-mm-dd',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;入职时间',
					name : 'ruzhishijian',
					id : 'ruzhishijian',
					anchor : '95%',
					allowBlank : false
				...
				items : [ {
					xtype : 'textarea',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;家庭住址',
					name : 'jiatingzhuzhi',
					id : 'jiatingzhuzhi',
					anchor : '95%',
					allowBlank : false
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('yuangongxingming').value = ManageObj.dataGridPan
				.getStore().getAt(rowIndex).get('yuangongxingming');
		ManageObj.operationFormPan.findById('yuangonggonghao').value = ManageObj.dataGridPan
				.getStore().getAt(rowIndex).get('yuangonggonghao');
		ManageObj.operationFormPan.findById('ruzhishijian').value = ManageObj.dataGridPan
				.getStore().getAt(rowIndex).get('ruzhishijian');
		ManageObj.operationFormPan.findById('jiatingzhuzhi').value = ManageObj.dataGridPan
				.getStore().getAt(rowIndex).get('jiatingzhuzhi');
		ManageObj.operationFormPan.findById('lianxidianhua').value = ManageObj.dataGridPan
				.getStore().getAt(rowIndex).get('lianxidianhua');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/renliziyuanguanli/neibuyuangongguanli.do?flag=doExportExcel";
	window.open(url);
};
