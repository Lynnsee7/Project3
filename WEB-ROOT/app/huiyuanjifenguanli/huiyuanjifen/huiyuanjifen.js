					...
					items : [
						{
							xtype : 'textfield',
							fieldLabel : '会员名称',
							name : 'huiyuanmingchengSearch',
							id : 'huiyuanmingchengSearch',
							anchor : '100%'
						} ]
				}
				,
					...
					items : [
						{
							xtype : 'textfield',
							fieldLabel : '会员积分',
							name : 'huiyuanjifenSearch',
							id : 'huiyuanjifenSearch',
							anchor : '100%'
						} ]
				}
				,
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		remoteSort : true,
		fields : [ 'huiyuanmingcheng', 'huiyuanjifen', 'id', 'itime', 'detail', 'deleteFlag' ],
		proxy : new Ext.data.HttpProxy({
			url : '/WEB-ROOT/app/huiyuanjifenguanli/huiyuanjifen.do',
			method : 'POST'
		}),
		baseParams : {
			flag : 'getJsonStore',
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		...
		bbar : pagingBar,
		columns : [{
			header : '<font color="blue">会员名称</font>',
			width : 60,
			align : 'center',
			sortable : true,
			...
			, {
				header : '<font color="blue">会员积分</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'huiyuanjifen'
			}
			, {
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.huiyuanmingchengSearch = document.all.huiyuanmingchengSearch.value;
	ManageObj.dataStore.baseParams.huiyuanjifenSearch = document.all.huiyuanjifenSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
	ManageObj.operationFormPan = new Ext.FormPanel({
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;会员名称',
					name : 'huiyuanmingcheng',
					id : 'huiyuanmingcheng',
					anchor : '95%',
					allowBlank : false
				} ]
				...
				, {
					...
					items : [ {
						xtype : 'textfield',
						fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;会员积分',
						name : 'huiyuanjifen',
						id : 'huiyuanjifen',
						anchor : '95%',
						value : '0',
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('huiyuanmingcheng').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('huiyuanmingcheng');
		ManageObj.operationFormPan.findById('huiyuanjifen').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('huiyuanjifen');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/huiyuanjifenguanli/huiyuanjifen.do?flag=doExportExcel";
	window.open(url);
};
