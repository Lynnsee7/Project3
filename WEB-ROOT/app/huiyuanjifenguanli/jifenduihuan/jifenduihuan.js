					...
					items : [
						{
							xtype : 'textfield',
							fieldLabel : '兑换物品',
							name : 'duihuanwupinSearch',
							id : 'duihuanwupinSearch',
							anchor : '100%'
						} ]
				}
				,
					...
					items : [
						{
							...
							format : 'Y-m-d',
							fieldLabel : '兑换日期',
							name : 'duihuanriqiSearch',
							id : 'duihuanriqiSearch',
							anchor : '100%'
						} ]
				}
				,
					...
					items : [
						{
							xtype : 'textfield',
							fieldLabel : '使用积分',
							name : 'shiyongjifenSearch',
							id : 'shiyongjifenSearch',
							anchor : '100%'
						} ]
				}
			]
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		remoteSort : true,
		fields : [ 'huiyuanhao', 'duihuanwupin', 'duihuanriqi', 'shiyongjifen', 'id', 'itime', 'detail', 'deleteFlag' ],
		proxy : new Ext.data.HttpProxy({
			url : '/WEB-ROOT/app/huiyuanjifenguanli/jifenduihuan.do',
			method : 'POST'
		}),
		baseParams : {
			flag : 'getJsonStore',
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		...
		columns : [ {
			...
			align : 'center',
			sortable : true,
			dataIndex : 'huiyuanhao'
		}
			, {
				header : '<font color="blue">兑换物品</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'duihuanwupin'
			}
			, {
				header : '<font color="blue">兑换日期</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'duihuanriqi'
			}
			, {
				header : '<font color="blue">使用积分</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'shiyongjifen'
			}
			, {
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.huiyuanhaoSearch = document.all.huiyuanhaoSearch.value;
	ManageObj.dataStore.baseParams.duihuanwupinSearch = document.all.duihuanwupinSearch.value;
	ManageObj.dataStore.baseParams.duihuanriqiSearch = document.all.duihuanriqiSearch.value;
	ManageObj.dataStore.baseParams.shiyongjifenSearch = document.all.shiyongjifenSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
	ManageObj.operationFormPan = new Ext.FormPanel({
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;会员编号',
					name : 'huiyuanhao',
					id : 'huiyuanhao',
					anchor : '95%',
					allowBlank : false
				...
				, {
					...
					items : [ {
						xtype : 'textfield',
						fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;兑换物品',
						name : 'duihuanwupin',
						id : 'duihuanwupin',
						anchor : '95%',
						allowBlank : false
				...
				, {
					...
					items : [ {
						xtype : 'datefield',
						format : 'Y-m-d',
						fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;兑换日期',
						name : 'duihuanriqi',
						id : 'duihuanriqi',
						anchor : '95%',
						allowBlank : false
				...
				, {
					...
					items : [ {
						xtype : 'textfield',
						fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;使用积分',
						name : 'shiyongjifen',
						id : 'shiyongjifen',
						anchor : '95%',
						allowBlank : false
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('huiyuanhao').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('huiyuanhao');
		ManageObj.operationFormPan.findById('duihuanwupin').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('duihuanwupin');
		ManageObj.operationFormPan.findById('duihuanriqi').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('duihuanriqi');
		ManageObj.operationFormPan.findById('shiyongjifen').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('shiyongjifen');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/huiyuanjifenguanli/jifenduihuan.do?flag=doExportExcel";
	window.open(url);
};
