...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore( {
		...
		remoteSort : true,
		fields : [ 'lanmumingcheng','chuangjianren','id','itime','detail','deleteFlag' ],
...
,{
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('lanmumingcheng').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('lanmumingcheng');
ManageObj.operationFormPan.findById('chuangjianren').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('chuangjianren');
