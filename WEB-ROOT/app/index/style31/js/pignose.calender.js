		...
		var Component = {
			init : function(options) {
				...
				this.settings = $.extend({
					...
					date: moment(),
					format: 'YYYY-MM-DD',
					weeks: languagePack.weeks.ch,
					monthsLong: languagePack.monthsLong.ch,
					months: languagePack.months.ch,
					multiple: false,
					toggle: false
				}, options);
				...
				for(var idx in _this.settings.weeks) {
					...
					var $unit = $(Helper.Format('<div class="{0} {0}-{2}">{1}</div>', Helper.GetSubClass('Week'), week, languagePack.weeks.ch[idx].toLowerCase()));
				...
				return this.each(function() {
					...
					var rendering = function() {
						...
						for(var i=0; i<firstWeekday; i++) {
							var $unit = $(Helper.Format('<div class="{0} {0}-{1}"></div>', Helper.GetSubClass('Unit'), languagePack.weeks.ch[i].toLowerCase()));
						...
						for(var i=dateManager.firstDay; i<=dateManager.lastDay; i++) {
							...
							var $unit = $(Helper.Format('<div class="{0} {0}-date {0}-{3}" data-date="{1}"><a href="#">{2}</a></div>', Helper.GetSubClass('Unit'), iDate.format('YYYY-MM-DD'), i, languagePack.weeks.ch[iDate.weekday()].toLowerCase()));
						...
						for(var i=lastWeekday+1;$unitList.length <= 7 * 5;i++) {
							var $unit = $(Helper.Format('<div class="{0} {0}-{1}"></div>', Helper.GetSubClass('Unit'), languagePack.weeks.ch[i % 7].toLowerCase()));
							$unitList = $unitList.add($unit);
						}
